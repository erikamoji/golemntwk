...
const ReputationABI = require('./abis/Reputation.json'); // Assuming this is the correct path
...
async function calculateNewScore(userAddress) {
    let totalScore = 0;
    try {
        const feedbacks = await reputationContract.methods.getFeedbacks(userAddress).call();
...
async function updateReputationScore(userAddress, newScore) {
    ...
    const data = reputationContract.methods.setReputation(userAddress, newScore).encodeABI();
    ...
    try {
        ...
        console.log(`Reputation score updated for ${userAddress}: ${receipt.transactionHash}`);
    } catch (error) {
        console.error(`Failed to update reputation score for ${userAddress}: ${error}`);
    }
}
...
reputationContract.events.FeedbackAdded({
    ...
    if (newScore !== null) { // Ensure score calculation was successful
